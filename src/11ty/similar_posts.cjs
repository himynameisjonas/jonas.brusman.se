...
module.exports = function (collection, path, tags) {
  ...
  return collection
    .filter((post) => {
      return (
        getSimilarTags(post.data.tags, tags, tagFrequency) > 0 &&
        post.data.page.inputPath !== path
      );
    })
    .sort((a, b) => {
      const similarTagsDiff =
        getSimilarTags(b.data.tags, tags, tagFrequency) -
        getSimilarTags(a.data.tags, tags, tagFrequency);
